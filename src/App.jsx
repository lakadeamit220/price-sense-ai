...
function App() {
  ...
  const showToast = (message) => {
    ...
    setNotifications(prev => [{ id: Date.now(), text: message, time: new Date() }, ...prev]);
  ...
  const handleSavePromotion = (promoData) => {
    const newPromo = {
      ...
      endDate: new Date(Date.now() + (promoData.inputs.duration * 24 * 60 * 60 * 1000)).toISOString(),
      ...promoData
    };
    setSavedPromotions([newPromo, ...savedPromotions]);
    showToast("Promotion successfully scheduled!");
    ...
    <>
      ...
      >
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-slate-200/50 dark:bg-slate-800 p-1 rounded-xl inline-flex shadow-inner">
            <button
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-lg text-sm font-bold transition-all focus:outline-none flex items-center gap-2 ${
                activeTab === 'calendar'
                  ? 'bg-white dark:bg-slate-700 text-primary-600 dark:text-primary-400 shadow-sm'
                  : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
              }`}
            >
              ...
              {savedPromotions.length > 0 && (
                <span className="bg-primary-100 dark:bg-primary-900/50 text-primary-700 dark:text-primary-300 py-0.5 px-2 rounded-full text-[10px]">
        ...
        {activeTab === 'calendar' ? (
          ...
          >
            <PromotionCalendar promotions={savedPromotions} />
          </motion.div>
        ) : (
